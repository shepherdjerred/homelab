...
export function createScoutDeployment(chart: Chart, stage: Stage) {
  ...
  // SeaweedFS S3 credentials (shared between stages)
  const seaweedfsCredentials = new OnePasswordItem(chart, "seaweedfs-s3-credentials", {
    spec: {
  ...
  const baseEnvVariables = {
    ...
    AWS_ACCESS_KEY_ID: EnvValue.fromSecretValue({
      secret: Secret.fromSecretName(chart, "aws-access-key-id", seaweedfsCredentials.name),
      key: "s3-access-key-id",
    }),
    AWS_SECRET_ACCESS_KEY: EnvValue.fromSecretValue({
      secret: Secret.fromSecretName(chart, "aws-access-key-secret", seaweedfsCredentials.name),
