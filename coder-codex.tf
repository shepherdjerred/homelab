...
variable "openai_api_key" {
  type        = string
  description = "OpenAI API key for Codex. Generate one at: https://platform.openai.com/api-keys"
  sensitive   = true
}
...
data "coder_parameter" "codex_model" {
  name         = "codex_model"
  display_name = "OpenAI Model"
  description  = "The OpenAI model to use for code generation."
  default      = "o4-mini"
  type         = "string"
  icon         = "/icon/openai.svg"
  mutable      = true
...
data "coder_parameter" "codex_approval_mode" {
  name         = "codex_approval_mode"
  display_name = "Approval Mode"
  description  = "Controls how Codex handles approvals for actions."
  default      = "suggest"
  type         = "string"
  icon         = "/icon/openai.svg"
  mutable      = true
  option {
    name  = "Suggest (review before applying)"
    value = "suggest"
  }
  option {
    name  = "Auto Edit (auto-approve file edits)"
    value = "auto-edit"
  }
  option {
    name  = "Full Auto (auto-approve all)"
    value = "full-auto"
  }
  order = 2
}
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "dind"
          image = "docker:dind@sha256:c9699f4fc7365e8daad3eb48ce3b62dc118b2a5267f7be7bcd3c4e4b2f1cfb4c"
          security_context {
            privileged = true
          }
          env {
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    # Install Codex CLI if not present in devcontainer
...
# Codex app for the Coder Tasks UI
resource "coder_app" "codex" {
  ...
  icon         = "/icon/openai.svg"
  command      = <<-EOT
    cd /workspaces && codex --model ${data.coder_parameter.codex_model.value} --approval-mode ${data.coder_parameter.codex_approval_mode.value} "${data.coder_task.me.prompt}"
  EOT
}
...
resource "coder_metadata" "container_info" {
  ...
  item {
    key   = "model"
    value = data.coder_parameter.codex_model.value
  }
  item {
    key   = "workspace image"
