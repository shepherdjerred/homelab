...
export class Precommit {
  /**
   * Runs pre-commit hooks on the provided source directory using Python and pre-commit.
   * Installs mise, bun, and python (with mise), and downloads kube-linter in parallel, then runs pre-commit with all tools available in PATH.
   * @param source The source directory to run pre-commit hooks against.
   * @param targetArch The target architecture for the kube-linter binary (default: "amd64").
   * @param kubeLinterVersion The version of kube-linter to use (default: "v0.6.8").
   * @returns The stdout from the pre-commit run.
   */
  @func()
  async preCommit(
    @argument({
