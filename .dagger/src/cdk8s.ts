...
export async function typeCheckCdk8s(source: Directory): Promise<string> {
  return getBaseContainer(source, "/workspace/src/cdk8s")
    ...
    .withMountedCache("/workspace/src/cdk8s/node_modules/.cache", dag.cacheVolume("typescript-cache"))
...
export async function buildK8sManifests(
  ...
  const builtContainer = getBaseContainer(source, "/workspace")
    ...
    // Cache CDK8s build artifacts and TypeScript compilation
    .withMountedCache("/workspace/src/cdk8s/node_modules/.cache", dag.cacheVolume("typescript-cache"))
    .withMountedCache("/workspace/src/cdk8s/.cdk8s", dag.cacheVolume("cdk8s-cache"))
